  ...
  ({
    ...
    useEffect(() => {
      ...
      function tick() {
        ...
        const isCursorInside = m.x >= 0 && m.x <= w && m.y >= 0 && m.y <= h;
        const targetEngagement = isCursorInside
          ? Math.max(0.22, Math.min(m.speed / 5, 1))
          : 0;
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (distSq < crSq && eng > 0.01) {
            ...
            if (isBulge) {
              const pull = 1 - dist / cr;
              const push = pull * pull * (p.bulgeStrength as number) * eng;
